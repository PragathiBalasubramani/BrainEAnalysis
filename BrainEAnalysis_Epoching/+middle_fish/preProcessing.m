...
for aligni = 1:length(indalign)
    ...
    [EEGset{aligni}, ProcessVars(aligni).rejTrials,ProcessVars(aligni).EpochTrials] = middle_fish.epoching(EEGmain, ProcessVars(aligni).timelim, ProcessVars(aligni).alignnum, taskname);
    EEGset{aligni}.setname = ProcessVars(aligni).alignname;
    %load behaviorfile
    behtabledata = readtable(behFile,'Sheet',1);
    [ProcessVars(aligni).behOutput] = middle_fish.behavior_Analysis(behtabledata); %, rejTrials, EpochTrials);
    %collect different trial groups / conditions
    [ProcessVars(aligni).trialset, ProcessVars(aligni).behtabledata] = middle_fish.behavior_events(behtabledata, ProcessVars(aligni).rejTrials, ProcessVars(aligni).EpochTrials);
    ...
    %the stim is organised in such a way to maintain congruent / incongruent
    %separation
    trialtype(find(ismember(trialtype,[1,2,3,4]))) = 1;
    trialtype(find(trialtype~=1)) = 2;
    trialtype_dummy = dummyvar(trialtype); %congruent and incongruent separation alone
